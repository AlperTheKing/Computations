...
void find_numbers(int start, int end, std::vector<Result>& results, std::vector<int>& non_multiples, std::unordered_set<int>& used_multipliers) {
    std::vector<Result> local_results;
    std::vector<int> local_non_multiples;
    ...
    for (int i = start; i <= end; i++) {
        ...
        if (!found) {
            local_non_multiples.push_back(i);
        }
    }
    ...
    non_multiples.insert(non_multiples.end(), local_non_multiples.begin(), local_non_multipliers.end());
...
int main() {
    ...
    std::vector<int> non_multiples;
    std::unordered_set<int> used_multipliers;

    for (int i = 0; i < num_threads; i++) {
        int start = i * chunk_size + 1;
        int end = (i == num_threads - 1) ? max_num : (i + 1) * chunk_size;
        threads.emplace_back(find_numbers, start, end, std::ref(results), std::ref(non_multiples), std::ref(used_multipliers));
    }
    ...
    // Write the numbers with no valid multiple to the file
    outfile << "\nNumbers with no valid multiple:\n";
    for (const auto& num : non_multiples) {
        outfile << num << ": No valid multiples found up to 1 million times its digit sum." << std::endl;
    }
