...
void process_range(long long start, long long end, long long& max_number, int& max_steps, int threads_per_block) {
    ...
    CUDA_CALL(cudaMemset(d_max_steps, 0, sizeof(int)));
    CUDA_CALL(cudaMemset(d_max_number, 0, sizeof(long long)));
    ...
    CUDA_CALL(cudaGetLastError());  // Check if kernel execution failed

    // Copy results back to host
    CUDA_CALL(cudaMemcpy(&max_steps, d_max_steps, sizeof(int), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(&max_number, d_max_number, sizeof(long long), cudaMemcpyDeviceToHost));
