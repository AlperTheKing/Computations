...
std::mutex triplet_mutex; // Declare the mutex globally

// Mutex for synchronizing access to shared data
std::mutex factor_mutex;
std::atomic<bool> factor_found(false);  // Atomic flag to indicate that a factor is found
std::atomic<uint64_t> shared_factor(0); // Atomic variable to store the found factor
...
// Parallelized Pollard's Rho Algorithm using Random123 for random number generation
void pollards_rho(uint64_t n, r123::Philox4x32_R<10>::key_type key, r123::Philox4x32_R<10>::ctr_type ctr, uint64_t& counter, uint64_t& found_factor) {
    if (n % 2 == 0) {
        found_factor = 2;
        return;
    }
    ...
    while (!factor_found.load() && d == 1) { // Stop if another thread found a factor
        ctr[0] = counter++;
        ...
        if (d != 1 && d != n) {
            std::lock_guard<std::mutex> guard(factor_mutex);
            if (!factor_found.load()) { // Check if no one else found a factor yet
                factor_found.store(true);
                found_factor = d;
                shared_factor.store(d);
            }
        }
    }
}
...
void factor(uint64_t n, std::vector<uint64_t>& factors, r123::Philox4x32_R<10>::key_type key, r123::Philox4x32_R<10>::ctr_type ctr, uint64_t& counter) {
    ...
    uint64_t found_factor = 0;
    factor_found.store(false);  // Reset the flag before parallel execution
    shared_factor.store(0);     // Reset the shared factor before parallel execution
    
    // Launch threads for parallelized Pollard's Rho
    std::vector<std::thread> threads;
    const int numThreads = std::thread::hardware_concurrency();
    
    for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back(pollards_rho, n, key, ctr, std::ref(counter), std::ref(found_factor));
    }
    ...
    uint64_t d = shared_factor.load(); // The found factor from any thread
